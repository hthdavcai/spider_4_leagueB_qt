...
class MySQLStorePipeline(object):
    ...
    def __init__(self):  # 初始化连接mysql的数据库相关信息
        if password == '-1':
            self.server = redis.Redis(host=host, port=port, db=6)
        else:
            self.server = redis.Redis(host=host, port=port, password=password, db=6)
        self.encoder = ScrapyJSONEncoder()

    # pipeline dafault function                    #这个函数是pipeline默认调用的函数
    def process_item(self, item, spider):
        if item.has_key('###spiderConfigString###'):
            currentDataDemo = DataDemoUtil.__getDataDemoFromString__(self.DataDemoUtil,
                                                                     item['###spiderConfigString###'])
        else:
    ...
    def _conditional_insert(self, item):
        try:
            ...
            if item_primaryKey_n and str(item_select_o).find('\'\'', 0, len(str(item_select_o))) == -1:
                insert_sql = "insert into " + classNameShort + " (" + str(
                    colname_insert_n) + ") values(" + colvalue_insert_v + ")"  # placeholder_insert_n
                ...
                if primyKeyHaveNull == 0:
                    self.server.rpush("insertSQLList", insertORupdateSql)
